...
export default async function handler(req, res) {
  ...
  if (res.socket.server.io) {
    res.socket.server.io.emit('new-notification', {
      id: snsMessage.MessageId,
      message: snsMessage.Message,
      timestamp: new Date().toISOString()
    });
  } else {
    console.error('Socket.IO server instance not found.');
    return res.status(500).json({ message: 'Error: Socket.IO server not initialized' });
  }
